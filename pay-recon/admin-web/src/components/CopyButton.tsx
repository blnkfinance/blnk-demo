...
export default function CopyButton({ value, label = "Copy", className = "" }: Props) {
  ...
  return (
    <button
      ...
      className={`inline-flex shrink-0 items-center justify-center rounded-md p-1 text-muted transition-colors hover:bg-brand/10 hover:text-brand ${className}`}
    >
      {copied ? (
...
export function CopyableValue({
  ...
  return (
    <div>
      ...
      <dd
        className={`mt-0.5 flex items-center gap-1.5 font-medium text-ink ${
          mono ? "font-mono" : ""
