...
export async function simulateTransactions(
    ...
    try {
        ...
        // Calculate date range (6 months ago to now) for transaction effective dates
        const endDate = new Date();
        const startDate = new Date(Date.now() - 6 * 30 * 24 * 60 * 60 * 1000);

        const transactions: any[] = [];

        // Group balances by currency so we can ensure inter-account transfers only happen between balances of the same currency, preventing currency mismatch errors
        ...
        const availableCurrencies = useInternalBalances
            ? ["USD", "EUR", "GBP", "AUD", "CAD"] // Default currencies if using internal balances
        ...
        // Execute bulk transactions in batches of 5000
...
function getRandomTransactionType(): TransactionType {
    ...
    return types[index]!; // Safe because types array is never empty
}
...
function buildTransaction(
    ...
    if (useInternalBalances) {
        ...
        // Use existing balances from the pool - currency is guaranteed to exist at this point due to check above
        ...
        if (type === "Inter") {
            // destinationBalance is guaranteed to be defined here because we check and throw if undefined above
            if (!destinationBalance) {
                throw new Error(`Destination balance not set for Inter transaction`);
            }
